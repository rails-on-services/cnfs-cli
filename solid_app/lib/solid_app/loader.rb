...
module SolidApp
  class << self
    def add_loader(name:, path:, notifier: nil, logger: nil)
      name = name.to_s
      loaders[name] ||= SolidApp::Loader.new(name: name, logger: logger)
  ...
  class Loader
    ...
    # Example: SolidApp.loaders.first.last.select(SolidApp::Core, 'app/models')
    def select(plugin = SolidApp, path = '')
