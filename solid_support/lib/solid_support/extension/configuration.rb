  ...
  class Extension
    class << self
      ...
      def initialize! # rubocop:disable Metrics/AbcSize
        ...
        before_configuration.each { |blk| blk.call(SolidSupport.config) }

        # Second configurable block to run. Called before frameworks initialize.
        before_initialize.each { |blk| blk.call(SolidSupport.config) }
        ...
        SolidSupport.extensions.values.each do |extension|
          extension.config.initializers.each { |init| init[:block].call(SolidSupport.config) }
        ...
        before_eager_load.each { |blk| blk.call(SolidSupport.config) }

        yield

        # Last configurable block to run. Called after frameworks initialize.
        # Here have access to all classes in app
        after_initialize.each { |blk| blk.call(SolidSupport.config) }
      end
    end
