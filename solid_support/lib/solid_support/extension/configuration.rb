  ...
  class Extension
    class << self
      ...
      def initialize! # rubocop:disable Metrics/AbcSize
        ...
        before_configuration.each { |blk| blk.call(SolidSupport.application.config) }

        # Second configurable block to run. Called before frameworks initialize.
        before_initialize.each { |blk| blk.call(SolidSupport.application.config) }

        # Run each extension's initializers; Initialziers do not have access to plugin classes
        SolidSupport.extensions.values.each do |extension|
          extension.config.initializers.each { |init| init[:block].call(SolidSupport.application.config) }
        ...
        before_eager_load.each { |blk| blk.call(SolidSupport.application.config) }

        yield

        # Last configurable block to run. Called after frameworks initialize.
        # Here have access to all classes in app
        after_initialize.each { |blk| blk.call(SolidSupport.application.config) }
      end
    end
    ...
    class Configuration
      ...
      def before_configuration(&block)
        SolidSupport::Extension.before_configuration << block if block
      end

      # Second configurable block to run. Called before frameworks initialize.
      def before_initialize(&block)
        SolidSupport::Extension.before_initialize << block if block
      end

      # Third configurable block to run
      def before_eager_load(&block)
        SolidSupport::Extension.before_eager_load << block if block
      end

      # Last configurable block to run. Called after frameworks initialize.
      def after_initialize(&block)
        SolidSupport::Extension.after_initialize << block if block
      end
