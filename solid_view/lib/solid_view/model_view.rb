...
module SolidView
  class ModelView < TTY::Prompt
    ...
    def initialize(**options)
      ...
      super(**SolidView.config.view_options.merge(options))
    end

    def create
      raise ViewError, 'Need to pass a model. this is a bug' unless model
      raise ViewError, 'Create can only be called on new instances' if model.persisted?
    ...
    def modify_type
      ...
      raise ViewError, "#{view_class_name} not found. This is a bug. Please report." unless view_class
    ...
    def edit
      raise ViewError, 'Need to pass a model. this is a bug' unless model
      raise ViewError, 'Edit can only be called on existing instances' unless model.persisted?
