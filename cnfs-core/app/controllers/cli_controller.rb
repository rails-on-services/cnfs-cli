    ...
    class << self
      def load
        shortcuts.each_pair do |key, klass|
          ...
          define_method("#{key}f") { CliController.cache["#{key}f"] ||= klass.first }
          define_method("#{key}l") { CliController.cache["#{key}l"] ||= klass.last }
        end
