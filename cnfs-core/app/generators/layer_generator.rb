...
class LayerGenerator < GeneratorBase
  def env
    template('env.erb', "#{write_path}/layer.env") if environment
  end

  def manifest
    layer.services.each do |service|
      klass = service.type.nil? ? 'ServiceGenerator' : "#{service.type}Generator"
      call(klass, write_path, target, layer, service)
    end
  end

  private

  def environment; layer.environment end
